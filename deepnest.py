...
class DeepIterator:
	...
	def __next__(self):
		...
		while (self.top) and (self.top[-1][1] >= len(self.top[-1][0])): self.top.pop()

		if not self.top: raise StopIteration()

		buf = self.top[-1]
		if len(buf) == 3:
			key = buf[2][buf[1]]; obj = buf[0][key]; self.obj = (key, obj)
		else:
			pos = buf[1]; obj = buf[0][pos]; self.obj = obj
		buf[1] += 1

		if   type(obj) in (frozenset, set, dict):
			self.top.append([obj, 0, sorted(obj.keys())])
